...
namespace CSharpBeginner.Code
{
    public class LoadingContentDemo : SyncScript
    {
        ...
        public override void Update()
        {
            ...
            {
                loadedMannequinModel = Content.Load<Model>("Models/SwordModel"); //загружаем из папки Assets (в обозревателе решений) модельку
            ...
            if (Input.IsKeyPressed(Keys.U))
            {
                if (loadedMannequinModel != null) {
                    Content.Unload(loadedMannequinModel); // выгружаем
                    loadedMannequinModel = null; // перезаписываем переменную
                }
            }
            ...
            if (Input.IsKeyPressed(Keys.C))
            {
                ...
                {
                    Entity.Scene.Entities.Remove(spawnedEntities.Pop()); // удаляем последний добавленный энтити
                }
            }
        ...
        {
            if (loadedMannequinModel != null) // если загрузили модель
            {
                // loadedMannequinModel содержит геометрию, материалы, информацию и тд, но это все не привязано к сцене.
                var modelComponent = new ModelComponent(loadedMannequinModel);
                // ModelComponent связывает с системой рендеринга, управляет видимостью объекта и тд. Кароче Загружает в сцену наш loadedMannequinModel


                var randomPosition = new Vector3(random.Next(-2, 4), 0, random.Next(-2, 2)); //рандом позиция

                var entity = new Entity("My new entity with a model component", randomPosition);  //создаем ентити (основа всего в ECS) 
                entity.Add(modelComponent); //и загружаем в него модель

                Entity.Scene.Entities.Add(entity); //добавляем на сцену наш entity

                spawnedEntities.Push(entity); // добавляем в стек наш объъект
                // стек это список по принципу Last In, First Out. Т.Е. при удалении элемента удаляется последний добавленный.
            }
        }
    }
}
